...
export default function App() {
  ...
  const loadHomeData = async (isRefresh = false) => {
    try {
      ...
      setProducts(Array.isArray(productRes.data?.results) ? productRes.data.results : (Array.isArray(productRes.data) ? productRes.data : []));
  ...
  const reviewCount = 0;
  const visibleCategoryCards = selectedCategory === 'All Categories'
    ? categories
    : categories.filter((card) => (card.category_name || card.name) === selectedCategory);
        ...
        <View style={styles.categorySplitView}>
          ...
          <View style={styles.categoryContentArea}>
            <ScrollView contentContainerStyle={styles.gridContent} showsVerticalScrollIndicator={false}>
              {visibleCategoryCards.filter((card) => (card.category_name || card.name || '').toLowerCase().includes(searchTerm.toLowerCase())).map((card) => {
                const categoryName = card.category_name || card.name;
                const visual = getCategoryIconVisual(categoryName);
                return (
                  <TouchableOpacity key={card.id || categoryName} style={styles.categoryGridCard} onPress={() => { setSelectedCategory(categoryName); }}>
                    <View style={[styles.categoryGridIconBadge, { backgroundColor: visual.bgColor }]}> 
                      <Text style={[styles.categoryIconGlyph, { color: visual.color }]}>{visual.emoji}</Text>
                    </View>
                    <Image source={{ uri: card.image_url || card.image || 'https://images.unsplash.com/photo-1522337360788-8b13dee7a37e?auto=format&fit=crop&w=800&q=80' }} style={styles.gridImage} />
                    <Text style={styles.gridCardTitle}>{categoryName}</Text>
                    <Text style={styles.gridCardCount}>{card.description || 'Live from the backend'}</Text>
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
  ...
  const renderOrdersScreen = () => {
    const getOrderImageUrls = (order: any) => {
      const itemSources = [order?.items, order?.products, order?.order_items, order?.line_items].find(Array.isArray);
      const urls: string[] = [];

      if (Array.isArray(itemSources)) {
        itemSources.forEach((entry: any) => {
          const imageUrl = entry?.image_url || entry?.image || entry?.product_image || entry?.product?.image_url || entry?.product?.image || null;
          if (imageUrl) {
            urls.push(imageUrl);
          }
        });
      ...
      if (!urls.length && Array.isArray(order?.image_urls)) {
        urls.push(...order.image_urls.filter(Boolean));
      }
    ...
    if (selectedOrder) {
      ...
      return (
        <View style={styles.ordersPage}>
          <View style={styles.screenHeaderNavy}>
            <TouchableOpacity style={styles.headerIconButton} onPress={() => setSelectedOrderId(null)}>
              <Text style={styles.headerBackArrow}>←</Text>
            </TouchableOpacity>
          ...
          <ScrollView contentContainerStyle={styles.orderListContent} showsVerticalScrollIndicator={false}>
            <View style={styles.orderDetailCard}>
              ...
              <Text style={styles.detailTotal}>UGX {Number(selectedOrder.total_amount || 0).toLocaleString('en-US')}</Text>
      ...
      <View style={styles.ordersPage}>
        ...
        <ScrollView contentContainerStyle={styles.orderListContent} showsVerticalScrollIndicator={false}>
          {filteredOrders.length ? filteredOrders.map((order) => {
            ...
            const orderDate = order.created_at ? new Date(order.created_at).toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'Pending';
            const orderImages = getOrderImageUrls(order);
              ...
              <View key={order.id} style={styles.orderCard}>
                ...
                <View style={styles.thumbnailRow}>
                  {visibleImages.length ? visibleImages.map((imageUrl, index) => (
                    <Image key={`${order.id}-${index}`} source={{ uri: imageUrl }} style={styles.thumbImage} />
                  )) : (
                ...
                <View style={styles.orderMetaRow}>
                  <Text style={styles.orderMetaText}>{order.payment_method || 'Pay on Delivery'}</Text>
                  <Text style={styles.orderTotalText}>UGX {Number(order.total_amount || 0).toLocaleString('en-US')}</Text>
                </View>
                <View style={styles.orderActionsRow}>
                  <TouchableOpacity style={styles.detailButton} onPress={() => setSelectedOrderId(order.id)}>
                    <Text style={styles.detailButtonText}>View Details</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.trackButton} onPress={() => setSelectedOrderId(order.id)}>
            ...
            <View style={styles.profileDetailCard}>
              <Text style={styles.profileDetailBody}>No orders have been placed yet. Your recent deliveries will appear here once you place an order.</Text>
            </View>
          )}
        </ScrollView>
      </View>
        ...
        <ScrollView style={styles.profilePage} contentContainerStyle={styles.profilePageContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.profileDetailCard}>
            {cartItems.length ? (
              ...
              <>
                <Text style={styles.profileDetailBody}>Your cart is empty. Add products from the home screen to see them here.</Text>
                  ...
                  <View style={styles.productContent}>
                    ...
                    <View style={styles.productActions}>
                      <TouchableOpacity style={styles.cartButton} onPress={(event: any) => { event?.stopPropagation?.(); setSelectedProduct(null); handleAddToCart(item.id, 1, item); }}>
                        <Text style={styles.cartButtonText}>{currentQty > 0 ? 'Add another' : 'Add to cart'}</Text>
                      </TouchableOpacity>
                      {currentQty > 0 ? (
...
const styles = StyleSheet.create({
  ...
  fullScreenPanel: { flex: 1, paddingBottom: 96, backgroundColor: '#1B2A4A' },
  splashContainer: {
    flex: 1,
  ...
  screenRoot: { flex: 1, backgroundColor: '#1B2A4A' },
  scrollContent: { flexGrow: 1, paddingBottom: 112, paddingHorizontal: 16, backgroundColor: '#1B2A4A' },
  ...
  heroCardShell: { width: '100%', marginHorizontal: 0, marginBottom: 20, borderRadius: 28, backgroundColor: '#FFFFFF', paddingHorizontal: 0, paddingVertical: 0, shadowColor: '#000', shadowOpacity: 0.12, shadowRadius: 18, shadowOffset: { width: 0, height: 8 }, elevation: 6, overflow: 'hidden', alignItems: 'center', justifyContent: 'center' },
  heroBodyShell: { marginHorizontal: 0, borderRadius: 28, backgroundColor: '#FFFFFF', paddingHorizontal: 0, paddingVertical: 0, overflow: 'hidden' },
  ...
  productCard: { width: 170, backgroundColor: '#FFFFFF', borderRadius: 20, marginRight: 12, overflow: 'hidden', shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 10, shadowOffset: { width: 0, height: 3 }, elevation: 3, position: 'relative' },
  productImage: { width: '100%', height: 110, backgroundColor: '#F5F5F5' },
  ...
  productActions: { marginTop: 8, gap: 8, alignItems: 'center', justifyContent: 'center', width: '100%' },
  cartButton: { backgroundColor: '#1B2A4A', borderRadius: 999, paddingVertical: 8, paddingHorizontal: 14, alignItems: 'center', alignSelf: 'center', borderWidth: 1, borderColor: '#E5E7EB', minWidth: 120 },
  cartButtonText: { color: '#FFFFFF', fontWeight: '800', fontSize: 12 },
  ...
  filterTabsRow: { backgroundColor: '#FFFFFF', paddingVertical: 10 },
  filterTabsContent: { paddingHorizontal: 16 },
  filterTab: { marginRight: 18, alignItems: 'center' },
  ...
  orderListContent: { paddingHorizontal: 16, paddingTop: 12, paddingBottom: 24 },
  orderCard: { backgroundColor: '#FFFFFF', borderRadius: 18, padding: 14, marginBottom: 12, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 8, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
  ...
  trackButton: { backgroundColor: '#F5821F', borderRadius: 999, paddingVertical: 10, alignItems: 'center', flex: 1 },
  ...
  primaryButton: { marginTop: 16, marginHorizontal: 16, backgroundColor: '#F5821F', borderRadius: 999, paddingVertical: 12, alignItems: 'center' },
  ...
  profilePageContent: { paddingBottom: 24 },
